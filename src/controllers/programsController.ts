...
} from '../models/programsResponse';
import {
  ProgramsResponse1,
  programsResponse1Schema,
} from '../models/programsResponse1';
import {
  ProgramsResponse4,
  programsResponse4Schema,
} from '../models/programsResponse4';
...
import { Programs404Error } from '../errors/programs404Error';
...
export class ProgramsController extends BaseController {
  ...
  async getPrograms(
  ...
  ): Promise<ApiResponse<ProgramsResponse>> {
    ...
    return req.callAsJson(programsResponseSchema, requestOptions);
  }
  ...
  async createProgram(
    body: Program,
    requestOptions?: RequestOptions
  ): Promise<ApiResponse<ProgramsResponse1>> {
    ...
    return req.callAsJson(programsResponse1Schema, requestOptions);
  }
  ...
  async getProgram(
    ...
    req.throwOn(404, Programs404Error, 'Program not found');
    req.authenticate(false);
    return req.callAsJson(programsResponse1Schema, requestOptions);
  }
  ...
  async deleteProgram(
    programId: number,
    requestOptions?: RequestOptions
  ): Promise<ApiResponse<ProgramsResponse4>> {
    const req = this.createRequest('DELETE');
    const mapped = req.prepareArgs({ programId: [programId, number()] });
    req.appendTemplatePath`/programs/${mapped.programId}`;
    req.throwOn(404, Programs404Error, 'Program not found');
    req.authenticate([{ bearerAuth: true }]);
    return req.callAsJson(programsResponse4Schema, requestOptions);
  }
}
